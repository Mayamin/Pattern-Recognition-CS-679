...
#for validation
with open('results/error_val_data_16_20.csv', 'w+', newline='') as file:
    writer = csv.writer(file)
    for row in error_poly_rbf_f123:
        writer.writerow(row)
df = pd.read_csv('results/error_val_data_16_20.csv', header=None)
flag_poly = 0
...
for i in range(total_fold_number):
    ...
    for d in D_parameters:
        for c in C_parameters:
            ...
            df.to_csv('results/error_val_data_16_20.csv', index=False, header=False, mode='w')
            flag_poly += 1
    flag_poly = 0
...
with open('results/error_val_data_16_20.csv', 'r') as file:
...
#for test data
with open('results/error_data_16_20.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    for row in error_poly_rbf_f123:
        writer.writerow(row)
df = pd.read_csv('results/error_data_16_20.csv', header=None)
...
for i in range(total_fold_number):
    ...
    if i == 0 : #for fold 1
        ...
        error = error_rate(poly_accuracy)
        df.loc[i,0] = error 
        df.to_csv('results/error_data_16_20.csv', index=False, header=False, mode='w')
