...
def load_eigen_stuff (file_name):
    ...
    with open(file_name, 'r') as csv_file:
        ...
        to_ret = np.zeros((eigen_values.shape[0], eigen_values.shape[0]))
...
def load_proj_coeffs (file_name):
    ...
    with open(file_name,'r') as csv_file:
        ...
        to_ret = np.zeros((320, len(train_labels)))
...
if __name__ == '__main__':
    ...
    test_images_matrix, test_labels = load_test_images('./data/fb_L', average_face_vector)

    print(f'{eigen_vector_matrix.shape}, {eigen_vals.shape}, {projected_coefficient_matrix.shape}, {train_labels.shape}, {average_face_vector.shape}, {test_images_matrix.shape}, {len(test_labels)}')

    # transform them
    test_images_matrix = transform_to_eigen_space(test_images_matrix, eigen_vector_matrix[:320, :])
    ...
    print(f'{get_maha_distance(test, projected_coefficient_matrix[:, 0], eigen_vals[:320]) < .001}')
    ...
    plt.ylabel('Acc. (%)')
    plt.title('CMC curve for low res images')
