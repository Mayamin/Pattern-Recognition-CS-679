...
def mahalanobis_dist (eig_vals, test_coefficients, train_coefficients):
    ...
    tmp2 = test_coefficients-train_coefficients
...
image = Image.open('../data/fa_L/00023_930831_fa.pgm')
...
for i in range (train_coefficient_matrix.shape[1]):
    ...
    if mahalanobis_dist(eigen_values,  test_coeff , train_coeff) < .001:
...
for file_name in os.listdir(path_lowRes_test):
    ...
    if img is not None:
        ...
        tmp -= low_res_averageFace

        test_coeffff.append(tmp) 

        # print(test_coefficients_matrix)

test_coefficients_matrix = np.zeros((len(test_coeffff[0]), len(test_coeffff)))

# print(test_coeffff[0])
# print(test_coefficients_matrix.shape)

for i in range(len(test_coeffff)):
    for j in range(len(test_coeffff[0])):
        test_coefficients_matrix[j, i] = test_coeffff[i][j]

# print(test_coefficients_matrix)

test_coefficients_matrix = np.transpose(np.atleast_2d(test_coefficients_matrix))

# print(test_coefficients_matrix[:,0])

print(test_coefficients_matrix.shape)
# # print(eigen_matrix.shape)

test_projection_coefficients = np.transpose(np.matmul(test_coefficients_matrix, eigen_matrix))
# print(test_projection_coefficients)

#for every test image in test folder compute distance and sort them
for j in range(test_projection_coefficients.shape[1]):
    ...
    test_coeff = test_projection_coefficients[:k, j]
    # print(test_coeff)
    for i in range (train_coefficient_matrix.shape[1]):
    #find min later on
        train_coeff = train_coefficient_matrix[:k,i]
        # print(train_coeff)
        # print("train_coeff.shape",train_coeff.shape)
        # print("test_coeff.shape",test_coeff.shape)

        dis = mahalanobis_dist(eigen_values[:k], test_coeff , train_coeff ) 
        # print()
